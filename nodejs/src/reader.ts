...
function parseMachine(f: h5wasm.File): Machine {
  const grp = asGroup(f.get("Machine"), "Machine");
  const a = grp.attrs;
  return {
    id: attrStr(a, "ID"),
    machine_name: attrStrReq(a, "Machine_Name"),
...
/** 0-indexed train number → `Machine/Optical_Trains/Optical_Train_NN/Optional_Components/ClearBox`. */
function clearboxPath(trainIndex: number): string {
  const trainId = `Optical_Train_${String(trainIndex + 1).padStart(2, "0")}`;
  return `Machine/Optical_Trains/${trainId}/Optional_Components/ClearBox`;
}
...
export class MachineConfigReader {
  ...
  /** Parse the HDF5 file and return a fully-typed MachineConfig. */
