/**
 * File_Version 1.1 stable model facade — navigation + get/set with SetMode.
 *
 * Deliberately independent of the previous version's facade — the handle
 * shapes below are generic (dict-navigation only, no on-disk-layout
 * knowledge), but are defined fresh here rather than shared.
 */
...
export class MachineConfigFileV1_1 implements MachineConfigFile {
  ...
  static async open(path: string): Promise<Result<MachineConfigFileV1_1, CapabilityError>> {
    try {
      const reader = new Hdf5AdapterV1_1(path);
      const config = await reader.parse({ includeBinary: true });
      const fv = (config.meta.file_version || '1.1').trim() || '1.1';
      if (fv !== '1.1') {
        return err(
          capabilityError(
            'UnsupportedVersion',
      ...
      return ok(new MachineConfigFileV1_1(asJson(config), path, fv));
    } catch (e) {
      return err(capabilityError('IoError', e instanceof Error ? e.message : String(e)));
    }
  }

  static create(version: string): Result<MachineConfigFileV1_1, CapabilityError> {
    const fv = version.trim() || '1.1';
    if (fv !== '1.1') {
      return err(
   ...
   * at once (in `details`) rather than failing on the first one. The
   ...
   * `trainIndex`. Converts the already-loaded in-memory clearbox object
   * rather than re-reading the file, so this works for both `open()`- and
