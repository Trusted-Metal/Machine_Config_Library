"""File_Version 1.1 stable model facade (session + get/set).

Deliberately independent of ``capabilities/v1_0/file.py`` — the handle
classes below are generic (dict-navigation only, no on-disk-layout
knowledge), but are still defined fresh here rather than imported, so v1.1
never depends on v1.0's module.
"""
...
class MachineConfigFileV1_1:
    ...
    @classmethod
    def open(cls, path: str | Path) -> Result["MachineConfigFileV1_1", CapabilityError]:
        try:
            adapter = Hdf5AdapterV1_1(str(path))
    ...
    @classmethod
    def create(cls, version: str = "1.1") -> Result["MachineConfigFileV1_1", CapabilityError]:
        import os
        import tempfile

        from machine_config.builder import MockConfigBuilder

        fv = version.strip() or "1.1"
        if fv != "1.1":
            return err(
                capability_error(
                    "UnsupportedVersion",
        ...
        # Build a plain v1.0-shaped mock config and hand it straight to
        # Hdf5WriterV1_1 — no separate migration step needed (Phase 2,
        # V1_1_IMPLEMENTATION_PLAN.md). The writer's own fallback derives
        # power_characterization from the mock's volts_to_watts_*/
        # watts_to_volts_* fields automatically, the same as it would for
        # any other v1.0-sourced model; avoids a second, independent
        ...
        os.close(fd)
        try:
            Hdf5WriterV1_1(config).write(tmp)
            opened = cls.open(tmp)
            if not opened.ok:
                return opened
    ...
    def opcua(self) -> Result["_NodeHandle", CapabilityError]:
        ...
        present but missing one or more required fields — same enforcement
        contract as v1.0's facade, see OPCUA_FIELD_PROMOTION_PLAN.md.
        """
        self._assert_open()
    ...
    def save(self, path: str | None = None) -> Result[None, CapabilityError]:
        ...
        try:
            ...
            Hdf5WriterV1_1(config).write(out)
            self._path = out
            return ok(None)
        except Exception as e:  # noqa: BLE001
            return err(capability_error("IoError", str(e)))
...
class _NodeHandle:
    def __init__(self, file: MachineConfigFileV1_1, getter, setter) -> None:
        self._file = file
        self._getter = getter
        self._setter = setter
...
class _TrainHandle:
    def __init__(self, file: MachineConfigFileV1_1, index: int) -> None:
        self._file = file
        self._index = index
...
class _TrainCollection:
    def __init__(self, file: MachineConfigFileV1_1) -> None:
        self._file = file
