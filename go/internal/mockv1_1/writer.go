// File_Version "1.1-mock" HDF5 writer — exact inverse of reader.go.
//
// A fully independent reimplementation of v1.0's writer
// (machine-config-go/capabilities/v1_0/hdf5/writer.go) with the 10
// documented differences from docs/migrations/mock_v1_0_to_v1_1.md applied
// natively, rather than delegating to v1.0's Write and patching the delta
// afterward in place. See reader.go's package-level doc comment for the
// full rationale.
package mockv1_1
...
// MockV1_1Writer serializes a MachineConfig to a File_Version "1.1-mock"
// HDF5 file.
type MockV1_1Writer struct {
	config *MachineConfig
}

// NewWriter constructs a writer for config.
func NewWriter(config *MachineConfig) *MockV1_1Writer {
	return &MockV1_1Writer{config: config}
}

// Write serializes w.config to path.
func (w *MockV1_1Writer) Write(path string) error {
	return write(w.config, path)
}
...
func write(cfg *MachineConfig, path string) error {
	...
	machineGrp, err := root.CreateGroup(RootMachine)
	if err != nil {
		return err
	}
	...
	for i := range cfg.OpticalTrains {
		trainGrp, err := trainsGrp.CreateGroup(TrainID(i))
		if err != nil {
			return err
		}
	...
	if cfg.Opcua != nil {
		opcuaGrp, err := root.CreateGroup(RootOPCUA)
		if err != nil {
			return err
		}
...
// nothing at all when v is false. Mirrors v1.0's identical convention for
// Scanner's four Invert_* fields.
...
func writeRootAttrs(g *h5c.Group, m *MachineConfigMeta) error {
	...
	// ADDITION (x2): typed in mock v1.1 (no v1.0 equivalent); written as ""
	// when nil so the file stays schema-complete, per the migration manifest.
	if err := ws(g, AttrFacilityID, strOrEmpty(m.FacilityID)); err != nil {
		return err
	}
	if err := ws(g, AttrConfigAuthor, strOrEmpty(m.ConfigAuthor)); err != nil {
		return err
	}
...
// writeMachine writes Machine/ attrs.
//
// REMOVAL (x2): Gas_Flow_Direction / Recoat_Direction are never written,
// regardless of what the in-memory model holds.
// NAME: Machine_Name -> Machine_Label.
// PATH / NAME+PATH (x4): build-plate values move into Machine/Dimensions/
// (Width/Height replacing X/Y_Dimension; Z_Dimension/Corner_Radius keep
// their v1.0 keys, just moved). Unit attrs stay on Machine/ unchanged.
func writeMachine(g *h5c.Group, ma *Machine) error {
	if err := ws(g, "ID", strOrEmpty(ma.ID)); err != nil {
		return err
	}
	if err := ws(g, AttrMachineLabel, ma.MachineName); err != nil {
		return err
	}
	...
	dimsGrp, err := g.CreateGroup(GroupDimensions)
	if err != nil {
		return err
	}
	defer dimsGrp.Close()
	if err := wf(dimsGrp, AttrBPWidth, ma.BuildPlateX); err != nil {
		return err
	}
	if err := wf(dimsGrp, AttrBPHeight, ma.BuildPlateY); err != nil {
		return err
	}
	if err := wf(dimsGrp, "Build_Plate_Z_Dimension", ma.BuildPlateZ); err != nil {
		return err
	}
	return wf(dimsGrp, "Build_Plate_Corner_Radius", ma.BuildPlateRadius)
}
...
func writeTrain(g *h5c.Group, t *OpticalTrain) error {
	...
	scannerGrp, err := g.CreateGroup(GroupScanner)
	if err != nil {
		return err
	}
	...
	lsGrp, err := g.CreateGroup(GroupLightSource)
	if err != nil {
		return err
	}
	defer lsGrp.Close()
	...
	colGrp, err := g.CreateGroup(GroupCollimator)
	if err != nil {
		return err
	}
	defer colGrp.Close()
	...
	scGrp, err := g.CreateGroup(GroupScannerCard)
	if err != nil {
		return err
	}
	defer scGrp.Close()
	...
	if t.OptionalComponents.Clearbox != nil {
		optGrp, err := g.CreateGroup(GroupOptionalComponents)
		if err != nil {
			return err
		}
		defer optGrp.Close()
		cbGrp, err := optGrp.CreateGroup(GroupClearBox)
		if err != nil {
			return err
		}
		defer cbGrp.Close()
...
// writeScanner writes Scanner/ attrs.
//
// NAME: the value moves from Working_Distance to Focal_Distance;
// Working_Distance_unit keeps its v1.0 key.
func writeScanner(g *h5c.Group, s *Scanner) error {
	...
	if err := wf(g, AttrFocalDistance, s.WorkingDistance); err != nil {
		return err
	}
...
func writeClearBox(g *h5c.Group, cb *ClearBox) error {
	...
	cdDS, err := g.CreateFloat64DatasetOpen(DSCorrectionData, []uint64{257, 257, 2}, cdFlat)
	if err != nil {
		return err
	}
	...
	icdDS, err := g.CreateFloat64DatasetOpen(DSInverseCorrectionData, []uint64{257, 257, 2}, icdFlat)
	if err != nil {
		return err
	}
	...
	// non-empty, matching v1.0's convention exactly.
...
// attributes plus its two compound datasets.
...
func writeSFCF(g *h5c.Group, sfcf *ScanFieldCorrectionFile) error {
	...
	ds, err := g.CreateUint8DatasetOpen(DSScanFieldCorrectionFile, data)
	if err != nil {
		return err
	}
...
func writeOpcua(g *h5c.Group, opcua *OpcuaConfig) error {
	...
	// Triggers_Enabled is stored as float64, not int — same as v1.0.
