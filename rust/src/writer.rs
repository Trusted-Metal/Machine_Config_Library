...
///
/// Produces an HDF5 file whose structure and attribute names mirror
/// [`crate::reader::MachineConfigReader`] exactly. Reading the output back
/// yields an identical `MachineConfig` (modulo `correction_data`/`raw_bytes`
/// when those were absent at parse time — they are written as zeros/empty).
